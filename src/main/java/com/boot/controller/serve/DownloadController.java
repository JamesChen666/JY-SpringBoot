...
import com.boot.model.Download;
...
public class DownloadController extends BaseController{
    ...
    private static final String BASE_PATH = "download";
    private static final String LIST = "download.list";

    @RequestMapping("/")
	public String index() {
		return BASE_PATH+"/download_list";
	}
    ...
    @RequestMapping("/add")
    public String add() {
        return BASE_PATH + "/download_add";
    }

     @RequestMapping("/edit")
    public String edit(HttpServletRequest httpServletRequest, ModelMap modelMap) {
        modelMap.put("id", httpServletRequest.getParameter("id"));
        return BASE_PATH + "/download_edit";
    }

    @ResponseBody
    @RequestMapping("/edit/{id}")
    public Object edit(@PathVariable Integer id) {
        Download Download = sqlManager.single(Download.class,id);
        return Download;
    }
    ...
    public AjaxResult save(HttpServletRequest request) {
        Download model = mapping(Download.class, request);
        if(model!=null){
            ...
            if(PublicClass.isNull(model.getId()+"")){
                ...
                Download download = sqlManager.single(Download.class,model.getId());
                model.setDownCount(download.getDownCount());
                //修改
    ...
    public AjaxResult delete(HttpServletRequest httpServletRequest) {
        ...
        for (String s : parameterMap.keySet()) {
            ...
            deleteById += sqlManager.deleteById(Download.class, id);
        }
        if (deleteById <= 0) {
            return fail(FAIL);
        }
    ...
    public AjaxResult disableOrEnable(HttpServletRequest httpServletRequest) {
        ...
        for (String s : parameterMap.keySet()) {
            if (!s.equals("flag")){
                ...
                Download single = sqlManager.single(Download.class, id);
                /*if (flag.equals("true")){
    ...
    public AjaxResult importExcel(MultipartHttpServletRequest request) {
        ...
        for (String s : multiFileMap.keySet()) {
            ...
            List<Download> Downloads = importExcel(file, Download.class);
            for (Download download : Downloads) {
                insert += sqlManager.insert(download);
            }
        }
        if (insert<=0){
            return fail(FAIL);
    ...
    public AjaxResult exportExcel(HttpServletRequest httpServletRequest) {
        ...
        if (ids == null||ids.isEmpty()) {
            mapList = sqlManager.select("download.list",Map.class);
        }else {
            mapList = appendToList("download.list",
                    SqlIntercepter.create().set("WHERE FIND_IN_SET(Id,#{ids})"),
                    Dict.create().set("ids", ids));
        }
        try {
            simpleExport("Download", mapList );
        }catch (Exception e){
            e.getStackTrace();
            return fail(FAIL);
        }
