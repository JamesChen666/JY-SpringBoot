    ...
    */
    ...
    $("#saveBtn").click(function(){
        ...
        saveSurvey(function(){
            isSaveProgress=false;
            notify("保存成功",1000);
            top.$.messager.alert("系统提示", "保存成功", "success");
        });
    ...
    $("#publishBtn").click(function(){
        ...
        /*saveSurvey(function(){
            isSaveProgress=false;
            window.location.href="/design/my-survey-design!devSurvey.action?surveyId="+questionBelongId;
        });*/
    ...
    function saveSurvey(callback){
        ...
        //if( svyNmSaveTag==="0" || svyNoteSaveTag==="0" || svyAttrSaveTag==="0"){
            ...
            //if(svyNmSaveTag==="0"){
                ...
                if(dwSurveyName=='' || dwSurveyName === undefined){
                    top.$.messager.alert("系统提示", "问卷名称必填", "error");
                    return;
                }
            ...
            //if(svyNoteSaveTag==="0"){
                ...
                if(dwSurveyNoteEdit=='' || dwSurveyNoteEdit === undefined){
                    top.$.messager.alert("系统提示", "问卷说明 必填", "error");
                    return;
                }
           ...
           // }
            ...
            var toGroup=$("select[name='toGroup']").val();
            var begTime=$("input[name='begTime']").val();
            var isForce=$("select[name='isForce']").val();
            ...
            data+="&ynEndNum="+ynEndNum+"&ynEndTime="+ynEndTime+"&endTime="+endTime+"&toGroup="+toGroup+"&isForce="+isForce+"&begTime="+begTime+"&endNum="+endNum;
            ...
            $.ajax({
                ...
                success:function(msg){
                    if(msg.flag){
                        ...
                        top.$.messager.alert("系统提示", "保存失败", "error");
                    }
                }
            });
        //}else{
...
function bindQuHoverItem(){
    ...
    $(".dwQuDelete").click(function(){
        ...
        if(confirm("确认要删除此题吗？")){
            ...
            if(quId!=""){
                var url="/survey_question/delete";
                var data="quId="+quId;
                $.ajax({
                    url:url,
                    data:data,
...
function editAble(editAbleObj){
    ...
    $("#dwCommonEditRoot .dwComEditMenuBtn").show();
    if(thClass.indexOf("quCoTitleEdit")>0){
        //题目标题
        $("#dwCommonEditRoot").addClass("quEdit");
        $("#dwCommonEditRoot .dwComEditMenuBtn").hide();
    }else if(thClass.indexOf("quCoOptionEdit")>0){
        //题目选项
        $("#dwCommonEditRoot").addClass("quOptionEdit");
        quItemBody.find("input")
    }else if(thClass.indexOf("dwSvyNoteEdit")>=0){
        //问卷欢迎语
...
function deleteRadioOption(){
    ...
    if(quOptionId!="" && quOptionId!="0" ){
        var url="/survey_qu_radio/delete";
...
function saveCheckbox(quItemBody,callback){
    ...
    if(saveTag==0){
        ...
        var url="/survey_qu_checkbox/save";
        var questionBelongId=$("#id").val();
        var quType=quItemBody.find("input[name='quType']").val();
        var quId=quItemBody.find("input[name='quId']").val();
        var orderById=quItemBody.find("input[name='orderById']").val();;
...
function deleteCheckboxOption(){
    ...
    var quOptionId=$(optionParent).find("input[name='quItemId']").val();
    if(quOptionId!="" && quOptionId!="0" ){
        var url="/survey_qu_checkbox/delete";
        var data="quItemId="+quOptionId;
        $.ajax({
            url:url,
            data:data,
...
function saveFillblank(quItemBody,callback){
    ...
    var saveTag=quItemBody.find("input[name='saveTag']").val();
    if(saveTag==0){
        var url="/survey_qu_fillblank/save";
        var questionBelongId=$("#id").val();
        var quType=quItemBody.find("input[name='quType']").val();
        var quId=quItemBody.find("input[name='quId']").val();
        var orderById=quItemBody.find("input[name='orderById']").val();;
...
function saveParagraph(quItemBody,callback){
    var saveTag=quItemBody.find("input[name='saveTag']").val();
    if(saveTag==0){
        var url="/survey_qu_paragraph/save";
        var questionBelongId=$("#id").val();
        var quType=quItemBody.find("input[name='quType']").val();
        var quId=quItemBody.find("input[name='quId']").val();
        var orderById=quItemBody.find("input[name='orderById']").val();;
...
function saveMultiFillblank(quItemBody,callback){
    var saveTag=quItemBody.find("input[name='saveTag']").val();
    if(saveTag==0){
        var url="/survey_qu_multifillblank/save";
        var questionBelongId=$("#id").val();
        var quType=quItemBody.find("input[name='quType']").val();
        var quId=quItemBody.find("input[name='quId']").val();
        var orderById=quItemBody.find("input[name='orderById']").val();
...
function deleteMultiFillblankOption(){
    ...
    var quOptionId=$(optionParent).find("input[name='quItemId']").val();
    if(quOptionId!="" && quOptionId!="0" ){
        var url="/survey_qu_multifillblank/delete";
        var data="quItemId="+quOptionId;
        $.ajax({
            url:url,
            data:data,
