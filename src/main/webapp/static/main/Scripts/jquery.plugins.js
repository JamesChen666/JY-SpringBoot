...
(function ($) {
    ...
    $.fn.tabs = function (options, param) {
        ...
        tabsContainer.bind('_resize', function () {
            var scrollWidth = tabsContainer.attr('scrollWidth');
            var width = $(document).width() - 250
            if (!scrollWidth)
                scrollWidth = tabsContainer[0].scrollWidth 
            if (tabsContainer.outerWidth() < scrollWidth) {
                tabsLeftScroll.show();
                tabsRightScroll.show();
                $(tabsContainer).width(width - 32)
            }
            else {
                tabsLeftScroll.hide();
        ...
        //标签事件
        $(tabsContainer).on("click", "div > span.tab", function () {
            selectTab($(this).attr('index'));
        })

        $(tabsContainer).on("mousedown", "div > span.tab", function (event) {

            if ($.support.msie && event.button == 4 || !$.support.msie && event.button == 1)
                if ($(this).attr('closable') == "true")
                    closeTab($(this).attr('index'));
        })
        $(tabsContainer).on("dblclick", "div > span.tab", function () {
            if ($(this).attr('closable') == "true")
                closeTab($(this).attr('index'));
        })
        $(tabsContainer).on("click", "div > span.tab > a.close", function () {
            closeTab($(this).attr('index'));
        })
    };
    ...
    function addTab(param) {
        ...
        if ($(tabsContainer).find("div").length > 0)
            html += '<a href="javascript:;" class="left" index="' + param.id + '" hidefocus="hidefocus"></a>'
        else
            html += '<a href="javascript:;" class="left first" index="' + param.id + '" hidefocus="hidefocus"></a>'


        if (param.closable)
            html += '<span href="javascript:;" id="tabs_link_' + param.id + '" class="tab" index="' + param.id + '" closable="' + param.closable + '" hidefocus="hidefocus">' + param.title + '<a href="javascript:;" class="close" index="' + param.id + '" hidefocus="hidefocus"></a></span>'
        else
            html += '<span href="javascript:;" id="tabs_link_' + param.id + '" class="tab" index="' + param.id + '" closable="' + param.closable + '" hidefocus="hidefocus">' + param.title + ' </span>'
        html += '<a href="javascript:;" class="right " index="' + param.id + '" hidefocus="hidefocus"></a>'
    ...
    function closeTab(id) {
        ...
        if ($(tabsContainer).find("div").find("a:first").hasClass("first") == false)
            $(tabsContainer).find("div").find("a:first").addClass("first")
